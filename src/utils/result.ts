 ...
 */
export type Result<T, U extends string | number = string> =
  | ResultSuccess<T>
  | ResultFailure<U>;
 ...
 */
export const resultSuccess = (<T>(value: T): ResultSuccess<T> => ({
  success: true,
  value,
})) as (<T>(v: T) => ResultSuccess<T>) & (() => ResultSuccess<void>);
 ...
 */
export const resultFail = <U extends string | number = string>(
  code: U,
  error?: any
): ResultFailure<U> => ({
  success: false,
